...
def main():
    ...
    # Only the probe head will be trained
    ...
    # In production SLB would replace this with their
    # actual labelled video dataset
    print("Creating placeholder dataset (random video-like data)...")
    dataset = [
        (
            # 8 frames of 64x64 RGB — matching our serving format
            torch.randint(0, 255, (8, 3, 64, 64), dtype=torch.uint8).numpy(),
            torch.randint(0, num_classes, (1,)).item()  # class label
        )
        for _ in range(50)
    ]
    ...
    # Step 7 — Log new model version to ML Repo
    # This creates decoder-classify-trained:1 in ML Repo
    # with a run ID linking back to this job
    print(f"Logging probe model to ML Repo as {output_name}...")
    mv = client.log_model(
        ml_repo="slb-pov",
        name=output_name,
